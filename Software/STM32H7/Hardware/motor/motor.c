...
void motor_init(void) {
    pid_init(&motor_l_speed_pid, 0.01f, 0.1f, 0.001f, 0.01f); // 初始化左电机 PID 速度控制器
    pid_init(&motor_r_speed_pid, 0.01f, 0.1f, 0.001f, 0.01f); // 初始化右电机 PID 速度控制器
    motor_l_speed_pid.mode = PID_MODE_SPEED; // 设置左电机为速度控制模式
    motor_r_speed_pid.mode = PID_MODE_SPEED; // 设置右电机为速度控制模式
}

void motor_l_set_speed(const int32_t speed) {
    motor_l_speed_pid.target_speed = (float) speed;
}

void motor_r_set_speed(const int32_t speed) {
    motor_r_speed_pid.target_speed = (float) speed;
}
