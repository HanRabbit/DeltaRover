...
#define MOTOR_L_PWM(pulse) { MOTOR_L_SWITCH(pulse); __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, pulse > 0 ? pulse : -pulse); }
#define MOTOR_R_PWM(pulse) { MOTOR_R_SWITCH(pulse); __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_2, pulse > 0 ? pulse : -pulse); }

#define MOTOR_UPDATE_FREQ 0.05f
